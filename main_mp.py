...
def worker(input_queue, output_queue, gpu1, gpu2, print_interval):
...
def printer(output_queue, result_file):
    ...
    while True:
        try:
            # print("trying to retrieve output")
            result = output_queue.get(timeout=5)
            if result == 'DONE':
                break
            idx, pred, correct, video = result
        except Empty:
            continue
...
def main(args):
    ...
    for i in range(args.num_workers):
        ...
        p = mp.Process(target=worker, args=(input_queue, output_queue, 2*i, 2*i+1, 10))
        processes.append(p)
        p.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logging_file", type=str, default="./logger.log")
    parser.add_argument("--output_file", type=str, default="./outputs/mp_output1.csv")
